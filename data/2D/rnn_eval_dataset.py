...
# %%
def get_input_data(df, seq_len, batch_size, num_batches, datadir=DATA_DIR):
...
def train_test_split(input_data, output_data, kalman_pred_data, ekf_pred_data, ukf_pred_data):
    test_idx = random.sample(range(len(input_data)), k = int(len(input_data)*0.2))
    train_idx = [i for i in range(len(input_data)) if i not in test_idx]

    test_input = input_data[test_idx]
    test_output = output_data[test_idx]

    train_input = input_data[train_idx]
    train_output = output_data[train_idx].reshape(-1, 3)
    ...
    return train_input, train_output, test_input, test_output, train_kalman_pred, test_kalman_pred, train_ekf_pred, test_ekf_pred, train_ukf_pred, test_ukf_pred
...
train_input, train_output, test_input, test_output, train_kalman_pred, test_kalman_pred, train_ekf_pred, test_ekf_pred, train_ukf_pred, test_ukf_pred = train_test_split(new_input_data, new_output_data, kalman_pred_data, ekf_pred_data, ukf_pred_data)
# %%
# # sanity check
